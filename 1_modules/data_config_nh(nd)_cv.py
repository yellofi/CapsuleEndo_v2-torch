...
def __main__(args):
    ...
    for name in label.index.values:  
        if label.loc[name]['source'] in sources:
            if (label.loc[name][args.ab_class] == 1 and label.loc[name]['negative'] == 0):
...
python3 data_config_nh(nd)_cv.py --target_sources p3_2 190814 negative --ab_class hemorrhagic --aug_frb 0 0 0 --save_name data_config_p3_2_negative_np-h_---_--_5f_cv

python3 data_config_nh(nd)_cv.py --target_sources p3_2 190814 negative --ab_class depressed --aug_frb 0 0 0 --save_name data_config_p3_2_negative_np-d_---_--_5f_cv
"""
