...
class Generator(th.nn.Module):
    ...
    def __init__(self, depth=7, latent_size=512, use_eql=True):
        ...
        from MSG_GAN.CustomLayers import GenGeneralConvBlock,             GenInitialBlock, _equalized_conv2d
...
class Discriminator(th.nn.Module):
    ...
    def __init__(self, depth=7, feature_size=512,
                 use_eql=True, gpu_parallelize=False, device_ids=[0]):
        """
        ...
        from MSG_GAN.CustomLayers import DisGeneralConvBlock,             DisFinalBlock, _equalized_conv2d
        ...
        self.rgb_to_features[self.depth - 2] =             from_rgb(self.feature_size // np.power(2, i - 2))
...
#                     self.rgb_to_features[i])
                self.layers[i] = th.nn.DataParallel(self.layers[i], device_ids = device_ids)
                self.rgb_to_features[i] = th.nn.DataParallel(
                    self.rgb_to_features[i], device_ids = device_ids)
...
#                                      use_eql=use_eql, gpu_parallelize=True).to(device)
            ...
            self.gen = DataParallel(self.gen, device_ids=[device.index])
            self.dis = Discriminator(depth, latent_size,
                                     use_eql=use_eql, gpu_parallelize=True, device_ids = [device.index]).to(device)
        else:
    ...
    def train(self, data, gen_optim, dis_optim, loss_fn, normalize_latents=True,
        ...
        assert isinstance(gen_optim, th.optim.Optimizer),             "gen_optim is not an Optimizer"
        assert isinstance(dis_optim, th.optim.Optimizer),             "dis_optim is not an Optimizer"
